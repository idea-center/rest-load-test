...
{
	def restBuilder = new RestBuilder()//(connectTimeout: 5000, readTimeout: 1800000)
	 ...
	 * @return
	 */
	def loadTestExistingReportsAngular()
	{
		log.info params
		def url = getBaseUrl(params.host, params.port) + '/v1/reports'
		...
		//{
			...
			if (status != 200)
			{
				def resp = [status: status, test: 'reports']
				render resp as JSON
				return
			}
			...
			def resp = [status: status, reportCount: reportCount, duration: duration, rate: (int)rate, totalReports: totalReports, test: 'reports']
			render resp as JSON
	...
	def loadTestExistingReportModelsAngular()
	{
		log.info params
		def url = getBaseUrl(params.host, params.port) + '/v1/reports'
		...
		if (!surveyIds)
		{
			def resp = [status: 'Connection timed out', test: 'reportModels']
			render resp as JSON
			return
		}
		...
		GParsPool.withPool SURVEY_REPORTS_THREADS, {
			surveyIds.eachParallel
			{
				...
				synchronized(reportIds)
				{
					...
					if (status != 200)
					{
						def resp = [status: status, test: 'reportModels']
						render resp as JSON
						return
					}
		...
		//render template: 'loadTestExistingReports', model: [status: status, reportCount: reportCount, duration: duration, rate: (int)rate, test: 'reportModels']
		def resp = [status: status, reportCount: reportCount, duration: duration, rate: (int)rate, test: 'reportModels']
		render resp as JSON
	}
	
	def loadTestExistingReportModelsAndQuestionsAngular()
	{
		log.info params
		def url = getBaseUrl(params.host, params.port) + '/v1/reports'
		...
		if (!surveyIds)
		{
			def resp = [status: 'Connection timed out', test: 'reportModelQuestions']
			render resp as JSON
			return
		}
		...
		GParsPool.withPool SURVEY_REPORTS_THREADS,
		{
			surveyIds.eachParallel
			{
				...
				synchronized(reportIds)
				{
					...
					if (status != 200)
					{
						def resp = [status: status, test: 'reportModelQuestions']
						render resp as JSON
						return
					}
		...
		def resp = [status: status, reportCount: reportCount, duration: duration, rate: (int)rate, reportsWithNoData: reportsWithNoData, test: 'reportModelQuestions']
		render resp as JSON
	}
}